...
function Pricing() {
    ...
    const plans = Array.isArray(plansData) ? plansData.map((p, i) => ({
        ...
        ctaHref: PRICING_PLANS[i]?.cta?.href || '#demo',
    })) : [];

    const calculatePrice = (plan) => {
        if (plan.priceValue === null || plan.priceValue === undefined) {
            return plan.price || (locale === 'zh' ? '聯繫我們' : 'Contact Us');
        }
    ...
    const getPeriod = (plan) => {
        if (plan.priceValue === null || plan.priceValue === undefined) return '';
